...
def build_sgd_optimizer(
    lossgrad, learningrate: float, momentum: float, wdecay: float
):
    ...
    def init(weightinit, netstate, rngkey):
        ...
        optstate["gm"] = jax.tree_map(
            lambda p: jnp.zeros(shape=p.shape), weightinit
        )
        optstate["alpha"] = learningrate

        return TrainState(optstate=optstate, netstate=netstate, rngkey=rngkey)

    def step(trainstate, minibatch, lrfactor):
        optstate = trainstate.optstate

        (loss, netstate), grad = lossgrad(
            optstate["w"], trainstate.netstate, minibatch, is_training=True
        )

        # momentum
        optstate["gm"] = jax.tree_map(
            lambda gm, g, w: momentum * gm + g + wdecay * w,
            optstate["gm"],
            grad,
            optstate["w"],
        )
        ...
        optstate["w"] = jax.tree_map(
            lambda p, gm: p - learningrate * lrfactor * gm,
            optstate["w"],
            optstate["gm"],
        )

        newtrainstate = trainstate._replace(
            optstate=optstate, netstate=netstate
        )
...
def build_ivon_optimizer(
    ...
    def step(trainstate, minibatch, lrfactor):
        ...
        (params, ivonoptstate) = optstate["w"], optstate["state"]
        n_samples = every_k
